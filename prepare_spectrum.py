...
def prepare_spectrum(data, err_scale=None, median_normalize=True, mask_misfits=False, mask_known_skylines=False):
    ...
    ###########################################################################
    data['flux'][np.isnan(data['flux'])] = 1
    data['error'][np.isnan(data['error'])] = np.nanmax(data['error'])
    if err_scale != None:
        data['error'] *= err_scale
    
    velscale = (299792.458*(np.log(data['wave'][-1]) - np.log(data['wave'][0])))/len(data['wave'])
    
    ###########################################################################
    """mask out all the pixels with fluxes we don't trust"""
    ###########################################################################
    
    # wave_inds = np.where((apogee_wave >= data['wave'][0]) & (apogee_wave <= data['wave'][-1]))[0]
    # chip_ends = np.array([wave_inds[0], wave_inds[-1]+1], dtype=int)

    return_mask = np.zeros(np.shape(data['wave']))
    masked_waves = data['wave'][~data['mask'].astype(bool)]
    
    skylines = known_skylines[np.where((known_skylines[:,0] >= data['wave'][0]) & (known_skylines[:,0] <= data['wave'][-1]))[0]][:,0]
    ...
    for i in range(len(data['wave'])):
        ...
        for j in range(len(masked_waves)):
            if data['wave'][i] < masked_waves[j]+dl and data['wave'][i] > masked_waves[j]-dl:
                return_mask[i] = 1
        
        """affected by skylines"""
        if mask_known_skylines:
            for j in range(len(skylines)):
                if data['wave'][i] < skylines[j]+ds and data['wave'][i] > skylines[j]-ds:
                    return_mask[i] = 1
                
        """consistently poorly fit by our models"""
        if mask_misfits:
            if (data['wave'][i] < 15395 and data['wave'][i] > 15360) or (data['wave'][i] < 16070 and data['wave'][i] > 16030) or (data['wave'][i] < 16240 and data['wave'][i] > 16210):
                return_mask[i] = 1
                

        """masking stuff near the fixed template chip gaps"""
        if (data['wave'][i] < apogee_wave[346]) or (data['wave'][i] > apogee_wave[3024] and data['wave'][i] < apogee_wave[3685]) or (data['wave'][i] > apogee_wave[5830] and data['wave'][i] < apogee_wave[6444]) or (data['wave'][i] > apogee_wave[8085]):
    ...
    if median_normalize:
        noise = data['error']/np.nanmedian(data['flux'][good_pixels])
        galaxy = data['flux']/np.nanmedian(data['flux'][good_pixels])

    prepped_spec = dict(gal = galaxy, err = noise, wav = data['wave'], pix = good_pixels, vsc = velscale, cnt = data['cont'], bad = bad_pixels, lim = data['lim'])
