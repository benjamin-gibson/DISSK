...
def split_chips(spectrum_flux, spectrum_wave, chip):
    count, stops = 0, []
    for d in range(len(spectrum_flux)):
        if spectrum_flux[d] != 0.0 and count % 2 == 0:
            count += 1
            stops.append(d)
        elif spectrum_flux[d] == 0.0 and count % 2 == 1:
            count += 1
            stops.append(d)
    if count > 6 or count == 0:
        print('CHIP SPLITTING FAILED')
        
    if chip == 'Blue':
        spec = spectrum_flux[np.arange(stops[0], stops[1], 1, dtype=int)]
        wave = spectrum_wave[np.arange(stops[0], stops[1], 1, dtype=int)]

    if chip == 'Green':
        spec = spectrum_flux[np.arange(stops[2], stops[3], 1, dtype=int)]
        wave = spectrum_wave[np.arange(stops[2], stops[3], 1, dtype=int)]
        
    if chip == 'Red':
        spec = spectrum_flux[np.arange(stops[4], stops[5], 1, dtype=int)]
        wave = spectrum_wave[np.arange(stops[4], stops[5], 1, dtype=int)]
...
def identify_chip_ends(spectrum):
...
def running_median(flux, bin_width, mask=[0]):
...
def unshift(flux, wave, vel, is_mask_array=False):
    ...
    f = interp1d(new_wave, flux, bounds_error=False, kind=kind)
    shift_flux = f(wave)
...
def identify_skylines_and_continuum(wave, flux, sky, fiber, bin_width, renormalize=True):
    ...
    for I in range(len(flux)-1):
        if I < 1 or I > len(flux) - 2 or flux[I] < 0 or np.isnan(flux[I]):
        ...
        if fiber >= 20 and fiber <= 35 and wave[I] >= 15890 and wave[I] <= 15960:
            ...
            skyline_mask[[I-1, I, I+1]] = False

        elif flux[I]/continuum[I] < -1 or flux[I]/continuum[I] > 3:
            skyline_mask[I] = False

    if renormalize:
        continuum = running_median(flux, bin_width, skyline_mask)
